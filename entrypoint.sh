...
# Persist GUI env for SSH/login shells
persist_gui_env() {
  local out="/etc/profile.d/45-gui-runtime.sh"
  if [ -n "${DISPLAY:-}" ] || [ -n "${WAYLAND_DISPLAY:-}" ] || [ -n "${XDG_RUNTIME_DIR:-}" ]; then
    {
      echo "# Generated by entrypoint to persist GUI env for login shells"
      [ -n "${DISPLAY:-}" ]         && echo "export DISPLAY='${DISPLAY}'"
      [ -n "${WAYLAND_DISPLAY:-}" ] && echo "export WAYLAND_DISPLAY='${WAYLAND_DISPLAY}'"
      [ -n "${XDG_RUNTIME_DIR:-}" ] && echo "export XDG_RUNTIME_DIR='${XDG_RUNTIME_DIR}'"
    } > "$out"
    chmod 0644 "$out"
    echo "[entrypoint] Persisted GUI env to $out"
  fi
}
